...
#ADDED BY US
def estimate_expected_influence(graph, influencers, haters, num_simulations=1000):
	# To calc Expectance of an influence (uses given 'simulate_influence' method)
    total_influenced = 0
    for _ in range(num_simulations):
        total_influenced += simulate_influence(graph, influencers, haters)
    expected_influence = total_influenced / num_simulations
    return expected_influence

def node_filttering(graph, costs, haters):
	# Chossing a sub-set of vertices that will be "available" to choose S_0 from.
	# First, removing ALL haters vertices.
	costs_with_no_haters = {user_id: key for user_id, key in costs.items() if user_id not in haters}
	# Seconed, removing all vertices with 0 neighbors, can not influence or be influenced.
	nodes_with_0_neighbors = {node for node, degree in graph.degree() if degree == 0}
	costs_no_haters_no_loners = {user_id: cost for user_id, cost in costs_with_no_haters.items()
								 if user_id not in nodes_with_0_neighbors}
	# Removing all vertices that have only haters as neighbors.
	all_friends_are_haters = {
		node for node in graph.nodes
		if all(neighbor in haters for neighbor in graph.neighbors(node))
	}
	filtered_costs = {user_id: cost for user_id, cost in costs_no_haters_no_loners.items()
					  if user_id not in all_friends_are_haters}
	# Creating a new "network" graph that contains only relevent vertices and computing all centrality values on it.
	new_network_graph = graph.subgraph(filtered_costs).copy()
	# Centrality stats.
	# Degree for each vertex.
	degrees = dict(new_network_graph.degree())
	# Harmonic Centrality - native NetworkX method does not "normalize" the sums.
	harmonic_centrality = nx.harmonic_centrality(new_network_graph)
	harmonic_centrality = {v : value/len(new_network_graph) for v , value in harmonic_centrality.items()}
	# Threshold calculations.
	degree_threshold = np.percentile(list(degrees.values()), 50)  # חצי הכי מחוברים
	harmonic_threshold = np.percentile(list(harmonic_centrality.values()), 50)
	# Filterring by degree \ harmonic thresholds.
	available_nodes = {
		u for u in filtered_costs
		if degrees.get(u, 0) >= degree_threshold
		   or harmonic_centrality.get(u, 0) >= harmonic_threshold
	}
	utility = lambda u: (0 if u not in graph or u not in available_nodes
						 else
						 sum(infection_probability(v,
												   {hater_id: haters[hater_id] for hater_id in haters if
													hater_id in graph.neighbors(v)})
							 for v in graph.neighbors(u)) / costs[u])
	utility_rates = {u: utility(u) for u in available_nodes}
	sorted_utility_rates = dict(sorted(utility_rates.items(), key=lambda item: item[1], reverse=True))
	x = 500
	top_x_keys = list(sorted_utility_rates.keys())[:x]
	available_nodes = set(top_x_keys)
	print(len(available_nodes))
	return available_nodes

def infection_probability(v, haters_in_v_neighbors):
    """
    Calculating the Probability(v will be infected by u | u is infected)
    :param v: vertex in graph
    :param haters_in_v_neighbors: a dict of haters who are v's neighbors, key is id: value is its weight
    :return: probability value (float)
    """
    product = 1.0
    for a in haters_in_v_neighbors:
        w_a = haters_in_v_neighbors[a]
        product *= (1 - w_a)
    return 0.2 * product

def hill_climbing_with_budget(graph, costs, haters, budget, num_simulations=100):
	# Our modified version of the algorithm.
	S = []
	total_cost = 0.00
	available_nodes = node_filttering(graph, costs, haters)

	while True:
		best_node = None
		best_gain = -1
		current_influence = estimate_expected_influence(graph, S, haters, num_simulations)

		for v in available_nodes - set(S):
			if total_cost + costs[v] > budget:
				continue
			new_influence = estimate_expected_influence(graph, S + [v], haters, num_simulations)
			mv = new_influence - current_influence
			score = mv / (costs[v] + 1e-5)
			if score > best_gain:
				best_gain = score
				best_node = v

		if best_node is None:
			print("No valid node found to add. Stopping.")
			break
...
if __name__ == '__main__':
	...
	haters = read_haters()
	costs = read_costs()
	# Finding S_0 by using our own version of the hill_climbing algorighm & filterring vertices by centrality values.
	selected_nodes =hill_climbing_with_budget(graph, costs, haters, BUDGET)

	# Saving S_0 we found as a csv file.
	submit_influencers(selected_nodes, 'l' , 'n', costs, haters)
